    ...
    <div className="animate-in fade-in zoom-in-95 duration-500 slide-in-from-bottom-4">
      <header className="flex items-center justify-between mb-10">
        ...
        <div className="flex items-center gap-4">
          {profile?.tier === 'free' && (
            <div className="text-sm bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 px-4 py-2 rounded-xl flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="rounded-2xl border border-white/5 bg-white/5 p-6 relative overflow-hidden group shadow-xl">
      ...
      <div className="grid grid-cols-1 gap-4">
        {groups.length === 0 ? (
          ...
          groups.map((group: any) => (
            <Link key={group.id} href={`/subscriptions/${group.id}`} className="p-6 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-colors group flex items-center justify-between shadow-lg">
              <div className="flex items-center gap-4">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center shadow-inner font-bold text-xl text-white"
              ...
              <div className="text-right">
                <p className="font-bold text-lg text-indigo-400">${Number(group.total_price).toFixed(2)}</p>
                <p className="text-xs text-zinc-500">Total Mensual</p>
              </div>
            </Link>
          ))
        )}
