...
export default async function ParticipantsPage() {
    const { participants, billingPeriod } = await getAllParticipants()
    ...
    return (
        <div className="animate-in fade-in zoom-in-95 duration-500 max-w-5xl mx-auto">
            <header className="mb-10">
                <h1 className="text-3xl font-bold tracking-tight mb-2">Participantes</h1>
                            ...
                            {participants.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="py-10 text-center text-zinc-500 italic">
                                        No tenés participantes. Creá un grupo y agregá amigos.
                                    </td>
                                </tr>
                            ) : (
                                participants.map((p) => (
                                    ...
                                    >
                                        <td className="py-4 px-6 font-medium text-white">{p.user_name}</td>
                                        <td className="py-4 px-6 text-zinc-300">{p.group.name}</td>
                                        <td className="py-4 px-6 text-indigo-300 font-medium">
                                            ${Number(p.quota_amount).toFixed(2)}
                                        </td>
